...
for cell in nb['cells']:
    if cell['cell_type'] == 'code':
        ...
        # 1. Update IGNORE_SIM_IDS
        if "IGNORE_SIM_IDS = []" in source:
            source = source.replace("IGNORE_SIM_IDS = []", "IGNORE_SIM_IDS = ['1205', '31', '1941']")
            cell['source'] = [line + '\n' for line in source.split('\n')]
            cell['source'][-1] = cell['source'][-1].rstrip('\n')
            
        # 2. Update create_overview_plots(summary_df)
        if "create_overview_plots(summary_df)" in source:
            old_str = "create_overview_plots(summary_df)"
            new_str = """# Generate separate overview plots for each track type
for track_type in sorted(summary_df['track_type'].dropna().unique()):
    ...
    if len(track_df) > 0:
        ...
        # 3. Update create_overview_plots(filtered, f' (>{MIN_PROGRESS}% Progress)')
        if "create_overview_plots(filtered, f' (>{MIN_PROGRESS}% Progress)')" in source:
            old_str = "create_overview_plots(filtered, f' (>{MIN_PROGRESS}% Progress)')"
            new_str = """for track_type in sorted(filtered['track_type'].dropna().unique()):
